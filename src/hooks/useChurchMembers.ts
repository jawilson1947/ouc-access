...
export function useChurchMembers() {
  ...
  const getAllMembers = async () => {
    ...
    try {
      ...
      const data = await response.json() as ApiResponse<ChurchMember[]>;
  ...
  const getPaginatedMembers = async (page: number) => {
    ...
    try {
      if (state.searchParams) {
        ...
        const data = await response.json() as ApiResponse<PaginatedResponse>;
        if (!data.success) throw new Error(data.error);
        setState(prev => ({
          ...prev,
  ...
  const createMember = async (memberData: CreateChurchMemberInput) => {
    ...
    try {
      ...
      const data = await response.json() as ApiResponse<{ EmpID: number }>;
      if (!data.success) throw new Error(data.error);
