...
function serveFallbackImage(req, res, requestedFilename) {
  console.log('🔄 Serving fallback image for:', requestedFilename);
  
  const fallbackPath = path.join(__dirname, 'public', 'images', 'PhotoID.jpeg');
  
  if (fs.existsSync(fallbackPath)) {
    console.log('✅ Fallback image found, serving PhotoID.jpeg');
    serveStaticFile(req, res, fallbackPath);
  } else {
    console.error('❌ Fallback image not found:', fallbackPath);
    res.writeHead(404, { 'Content-Type': 'text/plain' });
    res.end('Image not found and no fallback available');
  }
}
      ...
      try {
        ...
        if (req.url && req.url.startsWith('/images/')) {
          ...
          if (fs.existsSync(imagePath)) {
            ...
            console.warn('⚠️ Requested image not found:', req.url);
            console.warn('📋 Available images:', fs.readdirSync(imagesDir).join(', '));
            
            // Extract filename for fallback
            const filename = path.basename(req.url);
            serveFallbackImage(req, res, filename);
          }
          return;
        }
